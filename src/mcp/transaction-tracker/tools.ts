...
export const TxGetLast10BlocksTool: McpTool = {
    ...
    handler: async (agent: any, input: Record<string, any>) => {
        ...
        return {
            status: "instruction",
            task: "get_recent_blocks",
            message: "🧱 Get Last 10 Blocks - Cached API Approach",
            description: "Retrieve the 10 most recent blocks using cached Nodit API specifications",
            
            overview: {
                action: "Get the last 10 blocks from the blockchain",
                chain,
                purpose: "Simple test to verify block data retrieval"
            },

            execution_plan: [{
                step: 1,
                action: "Get last 10 blocks using block range",
                tool: "cached_nodit_api",
                parameters: {
                ...
                expected_result: "10 most recent blocks with basic information"
            }],

            block_processing: {
                data_extraction: [
                    "Extract block numbers and timestamps",
                    "Parse block hashes and parent relationships",
                    "Calculate block times and intervals",
                    "Count transactions per block"
                ],
                analysis: [
                    "Verify block sequence (consecutive numbers)",
                    "Calculate average block time",
                    "Identify blocks with most transactions",
            ...
            expected_output: {
                summary: {
                    ...
                    timestamp: "ISO timestamp",
                    blocks_retrieved: 10,
                    latest_block_number: "Highest block number",
                    block_range: "Range from oldest to newest block"
                },
                blocks: [
                    {
                        number: "Block number",
                ...
                chain_insights: {
                    ...
                    total_transactions: "Total transactions in 10 blocks",
                    average_gas_usage: "Average gas usage percentage",
